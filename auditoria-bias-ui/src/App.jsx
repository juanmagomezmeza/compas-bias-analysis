...
function App() {
  ...
  const handleFileUpload = (e) => {
    ...
    if (file) {
      ...
      Papa.parse(file, {
        preview: 1, // Solo leemos la primera fila
        complete: (results) => {
          const cols = results.data[0];
          setHeaders(cols);
          // Autocompletar el path del data en el form
  ...
  const onSubmit = (data) => {
    const finalConfig = {
      ...
      favorable_classes: [0], // Valor por defecto para COMPAS
      protected_attribute_names: [data.protected_col],
      privileged_classes: [[data.privileged_val_raw]],
      features_to_keep: headers, // Usamos todos los detectados por PapaParse
      plot_mapping: {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <header className="flex items-center gap-4 mb-10 border-b border-slate-800 pb-6">
          ...
          <div>
            ...
            <p className="text-slate-400">Generador de configuración para Auditoría de Sesgos</p>
          </div>
        </header>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          ...
          <section className="bg-slate-800 p-6 rounded-xl border border-slate-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input {...register("dataset_name")} placeholder="Ej: Análisis COMPAS ProPublica" className="w-full bg-slate-700 border-slate-600 rounded-lg p-2 focus:ring-2 focus:ring-blue-500 outline-none" />
              </div>
            </div>
          </section>

          {/* SECCIÓN 2: Mapeo de Variables (Solo aparece si hay headers) */}
          {headers.length > 0 && (
            <section className="bg-slate-800 p-6 rounded-xl border border-slate-700 animate-in fade-in duration-500">
            <h2 className="flex items-center gap-2 text-xl font-semibold mb-6">
              <BarChart3 className="text-emerald-400" /> 2. Mapeo de Atributos y Visualización
            </h2>
            
            {/* Fila 1: Configuración Core de AIF360 */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div>
                <label className="block text-sm text-slate-400 mb-2 font-medium">Variable Objetivo (Label)</label>
                <select {...register("label_name")} className="w-full bg-slate-700 border-slate-600 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-emerald-500">
              ...
              <div>
                ...
                <select {...register("protected_col")} className="w-full bg-slate-700 border-slate-600 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-emerald-500">
                  {headers.map(h => <option key={h} value={h}>{h}</option>)}
                </select>
              </div>
              ...
              <div>
                ...
                <input {...register("privileged_val_raw")} placeholder="Ej: Caucasian" className="w-full bg-slate-700 border-slate-600 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>
            </div>
          
            {/* Sección Plot Mapping (Específico para tus gráficos) */}
            <div className="pt-6 border-t border-slate-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                {/* Columna Secundaria (sex) */}
                <div>
                  <label className="block text-sm text-slate-400 mb-2 font-medium">Columna Secundaria (Filtro)</label>
                  <select {...register("secondary_col")} className="w-full bg-slate-700 border-slate-600 rounded-lg p-2.5 outline-none">
                    {headers.map(h => <option key={h} value={h}>{h}</option>)}
                  </select>
                </div>
                ...
                <div>
                  ...
                  <input {...register("unprivileged_val_name")} placeholder="Ej: African-American" className="w-full bg-slate-700 border-slate-600 rounded-lg p-2.5 outline-none" />
                </div>
          
                {/* Columna de Puntaje (decile_score) */}
                <div>
                  <label className="block text-sm text-slate-400 mb-2 font-medium">Columna de Riesgo (Score)</label>
                  <select {...register("risk_score_col")} className="w-full bg-slate-700 border-slate-600 rounded-lg p-2.5 outline-none">
                    {headers.map(h => <option key={h} value={h}>{h}</option>)}
                  </select>
                </div>
                ...
                <div>
                  ...
                  <input {...register("plot_mapping.target_label_name")} placeholder="Ej: Reincidencia" className="w-full bg-slate-700 border-slate-600 rounded-lg p-2.5 outline-none" />
                </div>
                ...
                <div className="flex items-center gap-4 bg-slate-900/50 p-4 rounded-lg border border-slate-700/50 md:col-span-2">
                  ...
                  <input type="number" {...register("plot_mapping.risk_threshold")} className="bg-slate-700 p-2.5 rounded-lg w-24 text-center text-emerald-400 font-bold border-slate-600 outline-none" />
                </div>
              </div>
            </div>
          </section>
          ...
          <button 
            ...
            className="w-full flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 disabled:bg-slate-700 disabled:cursor-not-allowed text-slate-900 font-bold py-4 rounded-xl transition-all shadow-lg shadow-emerald-500/20"
          >
            <Download /> Generar config.json
          </button>
